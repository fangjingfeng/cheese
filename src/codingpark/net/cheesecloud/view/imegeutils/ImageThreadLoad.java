package codingpark.net.cheesecloud.view.imegeutils;
...
public class ImageThreadLoad {
	...
	public void loadImage(String url, BaseAdapter adapter, ViewHolder holder) {
		System.out.println("loadImage==加载图片");
		resetPurgeTimer();
		Bitmap bitmap = getBitmapFromCache(url);// 从缓存中读取
		if (bitmap == null) {
			holder.fileThumb.setImageResource(R.drawable.image);//缓存没有设为默认图片
			ImageLoadTask imageLoadTask = new ImageLoadTask();
			imageLoadTask.execute(url, adapter, holder);
		} else {
			holder.fileThumb.setImageBitmap(bitmap);//设为缓存图片
		}
	}
	...
	class ImageLoadTask extends AsyncTask<Object, Void, Bitmap> {
		...
		protected void onPostExecute(Bitmap result) {
			...
			holder.fileThumb.setImageBitmap(result);
